...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box }
    ...
    .controls { display: grid; grid-template-columns: 1fr auto auto auto; gap: 10px; margin: 12px 0 2px }
    ...
    .leaflet-control-attribution { background: rgba(17,26,43,.8); color: #c6d7f2 }
...
NLPL ZAHEERABAD BRANCH\t17.691302, 77.639235`;
    ...
    function updateMarkers(filter) {
      ...
      if (bounds.length) {
        ...
        map.setView([15.5, 76.5], 6); // South India default view
      }
    }
    ...
    locateBtn.addEventListener('click', () => {
      ...
      navigator.geolocation.getCurrentPosition(pos => {
        ...
        const you = L.circleMarker([lat, lon], { radius: 6, color: '#00d4ff' }).addTo(map).bindPopup('You are here');
